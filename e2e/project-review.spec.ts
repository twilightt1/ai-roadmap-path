...
test("creates an immutable snapshot and completes an allow-listed change-request cycle", async ({ browser, page }) => {
  ...
  const evidenceRevision = Date.now().toString(36);
  ...
  await page.getByLabel("Reflection").fill(
    `Tôi tách phần chuẩn hóa dữ liệu khỏi mô hình để kiểm tra deterministic, so sánh metric rõ ràng và xử lý failure path mà không thay đổi domain contract. Revision ${evidenceRevision}.`
  );
  ...
  // The submission card hydrates independently from the evidence draft. Wait for
  // that hydration before probing optional status/buttons so a restored canary can
  // resubmit from the changes_requested state left by its previous successful run.
  await expect(page.getByTestId("project-submission-card").getByText("Submission snapshot", { exact: true }))
    .toBeVisible();
  ...
  if (await submitButton.count()) {
    ...
    const submitResponse = page.waitForResponse((response) =>
      response.url().includes("/rest/v1/rpc/submit_project_evidence")

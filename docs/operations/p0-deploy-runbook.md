...
#### Stage 3 exact command and automated notes/snippets regression
...
This must execute every anonymous journey plus both authenticated tests in `e2e/practice-library.spec.ts`:

1. `authenticated library › allows a learner to bookmark a challenge and view it in the library` normalizes reusable smoke-user bookmark state, saves the challenge durably, and verifies it in `/library`.
2. `authenticated library › persists and cleans up a lesson note and challenge snippet` creates and edits a disposable non-sensitive note, verifies it in `/library`, deletes it and verifies absence, then saves a disposable challenge snippet, verifies it in `/library`, and deletes it.

The Playwright summary must report zero skipped, zero flaky, and zero failed tests. Record only test-user aliases, timestamps, operation outcomes, counts, stable error codes, and redacted UI state; do not retain note or code text in evidence. Include the candidate SHA, migration-history reference, target/flag proof, and operator/reviewer. An unsuccessful operation, a missing expected library result, incomplete cleanup, or any required authenticated skip is **BLOCKED**. Mirror this evidence in the `Post-migration authenticated bookmarks, notes, and snippets regression` and Stage 3 rows of `docs/operations/p0-verification-report.md`.
...
| 3 — ladder | `NEXT_PUBLIC_P0_PRACTICE_LADDER=true` (retain `NEXT_PUBLIC_P0_WORKER_EXECUTION=true`) | On the **prebuilt/redeployed** worker=true, ladder=true target, after the Stage 3 secret/target validation below, run `pnpm exec playwright test e2e/anonymous-learning.spec.ts e2e/practice-library.spec.ts --project=chromium`. This executes the authenticated bookmark/note/snippet regression and cleanup in addition to anonymous smoke. Inspect only privacy-safe practice events and stable error codes. | Every anonymous and authenticated journey runs with **zero skips, zero flaky, and zero failures**; bookmark, note create/edit/delete, snippet save/delete, ladder/walkthrough, and personal-library flows pass with worker=true/ladder=true deployment/runtime proof and without challenge, library, or progress regression. |

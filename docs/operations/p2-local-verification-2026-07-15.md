...
| P2 flag states used by Playwright builds | `true` for the full journey; `false` for the rollback assertion |
...
- Catalog-bounded project IDs and finite database timestamps prevent unbounded row creation and client-invalid hydration state.
- Sanitized sync diagnostics that contain error class only, never URL or reflection content.
- Dedicated `p2` and `p2-rollback` staging-canary workflow options.
- pnpm 11.13.0 / Node 22.13+ toolchain so the dependency gate uses npm's supported bulk advisory endpoint.

## Passed gates

| Gate | Result |
|---|---|
| `pnpm check` | PASS — 118 lessons, 118 quizzes, 35 challenge files, 2 ladders; 40 test files and 165 tests; 242 routes built; 235 pages indexed |
| `pnpm test:db` | PASS — all P0 progress, P1 learning-profile, and P2 ownership/validation/LWW assertions, including catalog bounds and non-finite timestamp rejection |
...
| Local P2 rollback assertion | PASS — 1 passed; evidence panel absent while the feature checklist remains usable |
...
- Only IDs in the static 52-project catalog are accepted; PostgreSQL infinite timestamps are rejected.
- Manual-review readiness counts non-whitespace reflection characters, matching the documented rubric.
...
7. Rebuild/redeploy with only `NEXT_PUBLIC_P2_PROJECT_EVIDENCE=false`, run the `p2-rollback` canary, and confirm the additive migration and saved evidence remain intact.
